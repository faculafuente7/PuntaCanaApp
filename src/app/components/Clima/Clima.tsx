...
import { Cloud, CloudRain, CloudSnow, CloudSun, Sun, Wind, ChevronLeft, ChevronRight, Droplets, Umbrella, MapPin, RefreshCw } from 'lucide-react';
import styles from './Clima.module.css';

// Coordenadas de las ubicaciones
const LOCATIONS = {
  'Punta Cana': { lat: 18.5601, lon: -68.3725 },
  'Bayahibe': { lat: 18.3667, lon: -68.8333 }
};
...
const Clima = () => {
  ...
  const [selectedLocation, setSelectedLocation] = useState<string>('Punta Cana');
  ...
  // Efecto para cargar datos climáticos cuando cambia la ubicación seleccionada
  useEffect(() => {
    fetchWeatherData();
  }, [selectedLocation]);
  ...
  const fetchWeatherData = async () => {
    try {
      setLoading(true);
      const { lat, lon } = LOCATIONS[selectedLocation as keyof typeof LOCATIONS];
      
      // Open-Meteo es completamente gratuito y no requiere API key
      const response = await fetch(
        `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&daily=weathercode,temperature_2m_max,temperature_2m_min,apparent_temperature_max,precipitation_probability_max,windspeed_10m_max,uv_index_max&timezone=auto`
      );
      ...
      setError(null);
      // Resetear el índice cuando cambia la ubicación
      setStartIndex(0);
    } catch (err) {
      console.error('Error fetching weather data:', err);
      setError(`No pudimos cargar el pronóstico para ${selectedLocation}. Usando datos típicos.`);
      generarDatosMockParaUbicacion(selectedLocation);
    } finally {
      setLoading(false);
    }
  };

  // Función para generar datos de muestra basados en la climatología real
  const generarDatosMockParaUbicacion = (ubicacion: string) => {
    // Datos climáticos promedios por mes (bastante similares entre Punta Cana y Bayahibe)
    const climaPorMes = [
    ...
    // Ajustes sutiles según la ubicación
    const ajustes = ubicacion === 'Bayahibe' 
      ? { temp: -0.5, lluvia: 5 } // Bayahibe algo más fresco y ligeramente más lluvioso
      : { temp: 0, lluvia: 0 };   // Punta Cana como referencia
    ...
    const mockData: WeatherDay[] = Array.from({ length: 14 }, (_, i) => {
      ...
      const probabilidadLluvia = datosDelMes.lluvia + ajustes.lluvia;
      const esLluvioso = Math.random() * 100 < probabilidadLluvia;
      
      return {
        date,
        temp: {
          day: Math.round(datosDelMes.tempDia + variacionTemp + ajustes.temp),
          min: Math.round(datosDelMes.tempMin + variacionTemp + ajustes.temp),
          max: Math.round(datosDelMes.tempMax + variacionTemp + ajustes.temp),
        },
        weather: {
        ...
        chanceOfRain: esLluvioso ? 70 + Math.floor(Math.random() * 30) : Math.floor(Math.random() * probabilidadLluvia),
    ...
    <div className={styles.climaContainer}>
        ...
        <div className={styles.headerLeft}>
          ...
          <div className={styles.locationSelector}>
            {Object.keys(LOCATIONS).map(location => (
              <button
                key={location}
                className={`${styles.locationButton} ${selectedLocation === location ? styles.activeLocation : ''}`}
      ...
      {loading ? (
        <div className={styles.loadingContainer}>
          ...
          <p>Cargando pronóstico para {selectedLocation}...</p>
        </div>
      ) : error ? (
